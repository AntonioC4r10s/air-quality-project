...
for i, ax in enumerate(axs.flat):
    if i == 0:
        ax.plot(dados.index, dados['humidity\\'], color=cores[i])
        ax.set_ylabel('Umidade')
        ax.set_xlabel('Amostra')
        ax.set_title('Gráfico de Umidade')
    elif i == 1:
        ax.plot(dados.index, dados['temperature\\'], color='orange')
        ax.set_ylabel('Temperatura')
        ax.set_xlabel('Amostra')
        ax.set_title('Gráfico de Temperatura')
    elif i == 2:
        ax.plot(dados.index, dados['co_analogic\\'], color=cores[i-1])
        ax.set_ylabel('CO')
        ax.set_xlabel('Amostra')
        ax.set_title('Gráfico de CO')
    elif i == 3:
        ax.plot(dados.index, dados['co2_analogic\\'], color=cores[i-1])
        ax.set_ylabel('CO2')
        ax.set_xlabel('Amostra')
